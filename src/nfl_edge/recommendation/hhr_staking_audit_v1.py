"""Preregistered HHR-only staking audit. Does not select wagers."""
...
def base_units(model_confidence_probability: float) -> float:
    q = float(model_confidence_probability)
    if q >= 0.70:
        return 1.25
    if q >= 0.65:
        return 1.00
...
def hhr_stake(row: Mapping[str, Any]) -> HHRStake:
    ...
    q = _f(row, "model_confidence_probability")
    ...
    return HHRStake(
        ...
        heavily_juiced=pressure >= WARNING_PRESSURE,
    )
