...
def test_index_mobile_accessible_model_driven():
 h=(FRONTEND/'index.html').read_text(); assert 'width=device-width' in h and 'manifest.webmanifest' in h and 'data/latest.json' not in h; assert 'id="offer-book"' not in h; assert 'available at any sportsbook' in h; assert 'class="skip-link"' in h and 'aria-live=' in h; assert 'backend' not in h.lower(); assert 'nfl-edge-wordmark.png' in h and h.count('class="tab-icon"')==4; assert 'ui-motion.css' in h
def test_service_worker_never_caches_api_responses():
 s=(FRONTEND/'sw.js').read_text(); assert "url.pathname.startsWith('/api/')" in s; assert "fetch(request,{cache:'no-store'})" in s; assert "nfl-edge-shell-v10" in s; assert "./ui-polish.css" in s and "./ui-branding.css" in s and "./ui-motion.css" in s; assert "./assets/nfl-edge-wordmark.png" in s; shell=re.search(r'APP_SHELL=\[(.*?)\];',s,re.S).group(1); assert '/api/' not in shell
...
def test_branding_nav_and_ultra_warning_are_presentation_only():
 h=(FRONTEND/'index.html').read_text(); b=(FRONTEND/'ui-branding.css').read_text(); u=(FRONTEND/'ux.js').read_text(); assert 'class="app-logo"' in h and 'ui-branding.css' in h; assert '.tab .tab-icon' in b and 'font-size:1.42rem' in b; assert 'content:url("data:image/png;base64,' in b; assert 'Ultra staking' in u and 'does not increase win probability' in u and 'expected percentage return' in u and 'Only the dollar swings get larger' in u; assert 'data-ultra-confirm' in u and 'data-ultra-back' in u
def test_motion_setting_and_branded_launch_are_local_presentation_only():
 u=(FRONTEND/'ux.js').read_text(); b=(FRONTEND/'ui-branding.css').read_text(); m=(FRONTEND/'ui-motion.css').read_text(); assert 'nfl-edge-motion-v1' in u; assert 'Transitions & loading animation' in u; assert 'prefers-reduced-motion: reduce' in u; assert 'view-transition-in' in u; assert 'app-ready' in u; assert 'Loading the board' in b; assert '.motion-switch' in b; assert 'body::after' in m and '@keyframes yard-lines-move' in m; assert 'body::before{animation:none!important' in m; assert '@media(prefers-reduced-motion:reduce)' in m
