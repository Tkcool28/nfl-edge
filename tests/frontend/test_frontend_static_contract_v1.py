...
def test_service_worker_never_caches_api_responses():
 s=(FRONTEND/'sw.js').read_text(); assert "url.pathname.startsWith('/api/')" in s; assert "fetch(request,{cache:'no-store'})" in s; assert "nfl-edge-shell-v7" in s; assert "./ui-polish.css" in s; shell=re.search(r'APP_SHELL=\[(.*?)\];',s,re.S).group(1); assert '/api/' not in shell
