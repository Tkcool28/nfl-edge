...
def test_index_mobile_accessible_model_driven():
 h=(FRONTEND/'index.html').read_text(); assert 'width=device-width' in h and 'manifest.webmanifest' in h and 'data/latest.json' not in h; assert 'id="offer-book"' not in h; assert 'available at any sportsbook' in h; assert 'class="skip-link"' in h and 'aria-live=' in h; assert 'backend' not in h.lower(); assert 'nfl-edge-wordmark.png' in h and h.count('class="tab-icon"')==4
def test_service_worker_never_caches_api_responses():
 s=(FRONTEND/'sw.js').read_text(); assert "url.pathname.startsWith('/api/')" in s; assert "fetch(request,{cache:'no-store'})" in s; assert "nfl-edge-shell-v9" in s; assert "./ui-polish.css" in s and "./ui-branding.css" in s; assert "./assets/nfl-edge-wordmark.png" in s; shell=re.search(r'APP_SHELL=\[(.*?)\];',s,re.S).group(1); assert '/api/' not in shell
...
def test_motion_setting_and_branded_launch_are_local_presentation_only():
 u=(FRONTEND/'ux.js').read_text(); b=(FRONTEND/'ui-branding.css').read_text(); assert 'nfl-edge-motion-v1' in u; assert 'Transitions & loading animation' in u; assert 'prefers-reduced-motion: reduce' in u; assert 'view-transition-in' in u; assert 'app-ready' in u; assert 'Loading the board' in b; assert '@keyframes yard-drive' in b; assert '.motion-switch' in b; assert '@media(prefers-reduced-motion:reduce)' in b
