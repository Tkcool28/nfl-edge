...
def test_frontend_required_api_flow_is_backend_authoritative(tmp_path):
 ...
 logged=c.post('/api/v1/wagers',json={'source_type':'HEADLINE','product_version':p['product_version'],'lane':'BALANCED','actual_units':.5,'actual_dollars':'4.00','note':'frontend integration','idempotency_key':'front-balanced-1'});assert logged.status_code==201,logged.text;wid=logged.json()['wager']['wager_id'];o=c.get('/api/v1/product/latest').json()['headline_overlays']['balanced'];assert o['wager_logged'] and o['logged_wager_id']==wid and o['actual_dollars']=='4.00';assert c.get('/api/v1/wagers').json()['wagers'][0]['wager_id']==wid;assert c.get('/api/v1/profile').json()['bankroll']=='496.00'
 assert c.patch(f'/api/v1/wagers/{wid}',json={'status':'WON'}).json()['status']=='WON';assert c.get('/api/v1/wagers',params={'state':'settled'}).json()['wagers'][0]['wager_id']==wid;summary=c.get('/api/v1/bankroll').json();assert summary['current_bankroll']=='503.48';assert summary['realized_pl']=='3.48';assert summary['open_stakes']=='0.00'
 ...
 cookies=dict(c.cookies);r=TestClient(app);r.cookies.update(cookies);assert r.get('/api/v1/auth/me').status_code==200;assert r.get('/api/v1/profile').json()['bankroll']=='503.48';assert r.get(f'/api/v1/wagers/{wid}').json()['actual_dollars']=='4.00'
