...
# v1 is SUPERSEDED by Task04B v2.  It carries a known postseason chronology
# defect (the frozen QB-stat source uses a broad "POST" label while the v1
# block logic expects WC/DIV/CON/SB), which can exclude valid earlier-playoff
# history for later postseason targets.  v1 artifacts are NOT authoritative and
# are NOT consumed by Task04C / any current downstream path.
# To guard against silently regenerating a misleading v1 reconstruction, the
# builder now refuses to run unless the operator explicitly opts in.
_SUPERSEDED_ENV = "NFL_EDGE_ALLOW_SUPERSEDED_V1"
_SUPERSEDED_MESSAGE = (
    "build_oracle_qb_entering_state_v1 is SUPERSEDED by Task04B v2 and refused to run: "
    "the v1 postseason chronology is defective (broad POST vs WC/DIV/CON/SB) and v1 is "
    "not the authoritative oracle reconstruction. Use "
    "scripts/build_oracle_qb_entering_state_v2.py instead. Re-run with "
    f"{_SUPERSEDED_ENV}=1 only if you explicitly need a legacy v1 artifact "
    "for historical comparison."
)


def _deprecation_guard() -> None:
    if os.environ.get(_SUPERSEDED_ENV) != "1":
        print(_SUPERSEDED_MESSAGE, file=sys.stderr)
        raise SystemExit(2)
