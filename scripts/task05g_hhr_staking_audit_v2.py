...
from nfl_edge.recommendation.staking_v1 import MINIMUM_STAKE_DOLLARS, PER_WAGER_BANKROLL_CAP_PCT, ROUNDING_QUANTUM_DOLLARS, risk_profile
...
def audit_dollar_stake(bankroll, profile, units):
    if float(units) not in {0.25,0.5,0.75,1.0,1.25,1.5}: raise ValueError('unexpected HHR audit units')
    b=Decimal(str(bankroll)); selected=risk_profile(profile)
    raw=b*Decimal(str(selected.unit_bankroll_pct))*Decimal(str(float(units)))
    cap=b*Decimal(str(PER_WAGER_BANKROLL_CAP_PCT)); bounded=min(raw,cap); q=Decimal(str(ROUNDING_QUANTUM_DOLLARS))
...
def run(v3, discovery, confirmation, out):
    ...
    dist=dict(sorted(Counter(float(r['hhr_units']) for r in picks).items())); warnings=[r for r in picks if r['heavily_juiced']]
    by_units={}
    for u in sorted({float(r['hhr_units']) for r in picks}):
        rr=[r for r in picks if float(r['hhr_units'])==u]; w=sum(settle(r)=='WIN' for r in rr); l=sum(settle(r)=='LOSS' for r in rr); p=sum(settle(r)=='PUSH' for r in rr)
    ...
    for profile in ['Cautious','Conservative','Normal','Aggressive','Ultra']:
        ...
        for r in picks:
            stake=audit_dollar_stake(bank,profile,float(r['hhr_units'])); pnl=stake*ppu(r); bank+=pnl; staked+=stake; profit+=pnl; peak=max(peak,bank); maxdd=max(maxdd,(peak-bank)/peak if peak else 0.0)
        profiles.append({'profile':profile,'ending_bankroll':bank,'profit':profit,'total_staked':staked,'max_drawdown_pct':maxdd})
    score={'version':'task05g_hhr_staking_audit_v2','confidence_input':'selector_trust','seasons':sorted(ALL),'sealed_not_run':[SEALED],'hhr_selected_count':len(picks),'unit_distribution':dist,'heavily_juiced_count':len(warnings),'heavily_juiced_rate':len(warnings)/len(picks) if picks else None,'pressure_min':min(float(r['hhr_price_pressure']) for r in picks),'pressure_median':float(pl.Series([float(r['hhr_price_pressure']) for r in picks]).median()),'pressure_max':max(float(r['hhr_price_pressure']) for r in picks),'by_units':by_units,'profiles':profiles,'invariants':{'all_selected_hhr_positive_units':all(float(r['hhr_units'])>0 for r in picks),'selector_result_count_matches_frozen_hhr':len(picks)==81,'no_2025':True}}
    out.mkdir(parents=True,exist_ok=True); (out/'scorecard.json').write_text(json.dumps(score,indent=2,sort_keys=True)+'\n'); pl.DataFrame(picks).sort('block').write_csv(out/'hhr_rows.csv')
